function [P,finish]=Path_search_A_star(Map,Start_point,End_point)
P=[];
finish=0;
[a,b]=size(Map);
Open_list=[];
...
if Map(index_i)==0 && Map(index_e)==0
    if norm(Start_point-End_point)~=0
        ...
        d_open(index_i,1)=norm(Start_point-End_point);
        g_open(index_i,1)=0;
    else
        P=index_i;
        return
