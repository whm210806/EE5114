...
for i = 1:size(output_temp,1)
	...
	for j = 1:particles_count
        ...
        detX=motion_estimate(i).x*(1+translation_noise_proportion*randn(1))+translation_noise_add*randn(1);
        detY=motion_estimate(i).y*(1+translation_noise_proportion*randn(1))+translation_noise_add*randn(1);
        detC=motion_estimate(i).theta*(1+theta_noise_proportion*randn(1))+theta_noise_add*randn(1);
        particles(j).theta=particles(j).theta+detC;
        particles(j).x=particles(j).x+cos(particles(j).theta)*detX-sin(particles(j).theta)*detY;
        particles(j).y=particles(j).y+sin(particles(j).theta)*detX+cos(particles(j).theta)*detY;
